...
const getFireBase = firebase.database().ref('en').on('value', (elem)=> {
	...
	for (let elem in allInfo) {
			...
			allInfo[elem]['items'].forEach (item => {
				...
				<div class="cases__item">
					...
					<div class="cases__item-box">
						...
						<p class="cases__item-text">${item.text}</p>
					</div>
		...
		if(elem == 'products') {
			...
			allInfo[elem]['buttons'].forEach (item => {
				btnsList += 
				`
					<button  class="products-btn" >${item}</button>
				`;
			});
		...
		if(elem == 'templates') {
			...
			allInfo[elem]['items'].forEach (item => {
				itemList += 
				`
				<div class="templates__item templates__item--en">
